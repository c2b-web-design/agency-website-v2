...
**⛔⛔ CB-CEILING-DROP — unresolved in the pinned data itself, and NOT fixable in code.** `wall-card-corners-4-september.md` lists it under *"What is still open"*: **CB's TR sits at y = 0, hard against the pinning tool's top edge**, while CA's TL is at 0.02849 — so Carl's rule *"The distance from the ceiling must be the same for CA and CB. Its like hanging a picture"* **was never satisfied.** ⚠ The old letterboxed framing hid it; the corrected framing shows it. ⛔ **Waiting on Carl to re-pin CB in `/proto/wall`. Do not invent the number** — that file says these are his, set by eye, and the Builder lost them twice already.
